...
class MSATransformer(nn.Module):
    ...
    def forward(
        ...
        if need_head_weights:
            ...
            if return_contacts:
                contacts,attent = self.contact_head(
                    tokens, row_attentions
                )
                att_avg = torch.mean(attent, -1, keepdim=False)
                att_max = torch.max(attent, -1, keepdim=False)[0]
                att_min = torch.min(attent, -1, keepdim=False)[0]
                result["contacts"] = contacts
                result["row_attentions"] = attent
                result["attentions_avg"] = att_avg
                result["attentions_max"] = att_max
                result["attentions_min"] = att_min
        return result
    def pretrain_attention(self, tokens):
        return self(tokens, return_contacts=True)
    def predict_contacts(self, tokens):
        return self(tokens, return_contacts=True)["contacts"]
    def get_attention_feature(self, tokens):
        return self(tokens, return_contacts=True)["row_attentions"]
    @property
    def num_layers(self):
        return self.args.layers
