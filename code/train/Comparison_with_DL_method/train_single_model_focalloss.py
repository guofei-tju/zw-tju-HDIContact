...
saved_models = "F:/ubuntu_file/result/model_result_again/"
train_dataset_add = "F:/ubuntu_file/data/Baker_dataset/baker31_dataset_min10.cpkl"
val_dataset_add = "F:/ubuntu_file/data/Ecoil_valdataset_min10_2.cpkl"
test_dataset_add = "F:/ubuntu_file/data/Ecoil_testdataset_min10_2.cpkl"
baker_attention_add = "F:/ubuntu_file/data/baker_feature/"
ecoil_attention_add = "F:/ubuntu_file/data/Ecoil_feature/"
model_type = "TurnBiLSTM128_1_focal"#"Conv_layer5"#"Conv_layer3" TurnBiLSTM
stop_stype = "acc"#Loss
hidden_size=128
hidden_size2 = 32
num_layer=1
#saved_models = "/home/mqz/esm-MSA/model_result"
epochs = 800  # 200
...
def adjust_learning_rate(optimizer, lr):
    for param in optimizer.param_groups:
        param["lr"] = lr
def adjust_loss_weight(optimizer, lossweight):
    for param in optimizer.param_groups:
        param["lossweight"] = lossweight
def DownSample_mask2(Map_Label, pnratio, mn, l_length):  # 返回有效样本区，正负样本比 mn = true label为mxn的

    if mn:  # m x n
        label = Map_Label[:l_length, l_length:]
    else:
        label = Map_Label
    map_size = label.shape[0] * label.shape[1]
    Tnum = int(sum(sum(label == 1)))
    Pnum = map_size - Tnum
    sample_Pnum = min(int(Tnum / 0.1), Pnum)
    ...
    for i in range(label.shape[0]):
        for j in range(label.shape[1]):
            if label[i, j] == 0:
                map_list_l.append(i)
                map_list_r.append(j)
    array = random.sample(range(0, Pnum), sample_Pnum)  # 1 -> 0 的数量
    Sample_mask = copy.deepcopy(label)
    for i in array:
        Sample_mask[map_list_l[i], map_list_r[i]] = 1
    return Sample_mask, Tnum / sample_Pnum, label
def get_aupr(rec_ls, pre_ls):
    ...
    for ix in range(len(rec_ls[:-1])):
        ...
        pr_value += temp_area
    return pr_value
def DownSample_mask(Map_Label):  # 返回有效样本区，正负样本比
    map_size = Map_Label.shape[0] * Map_Label.shape[1]
    Tnum = int(sum(sum(Map_Label == 1)))
    Pnum = map_size - Tnum
    if Tnum*10<Pnum:
        sample_Pnum = Tnum*10
        # 统计负样本位置
        map_list_l = []
        map_list_r = []
        for i in range(Map_Label.shape[0]):
            for j in range(Map_Label.shape[1]):
                if Map_Label[i, j] == 0:
                    map_list_l.append(i)
                    map_list_r.append(j)
        array = random.sample(range(0, Pnum), sample_Pnum)  # 1 -> 0 的数量
        Sample_mask = copy.deepcopy(Map_Label)
        for i in array:
            Sample_mask[map_list_l[i], map_list_r[i]] = 1
    else:
        sample_Pnum = Pnum
        Sample_mask = np.ones((Map_Label.shape[0] , Map_Label.shape[1]))
    return Sample_mask, Tnum / sample_Pnum
class Masked_BCELoss(nn.Module):
    ...
    def forward(self, pred, label, weight_mask):
        ...
        loss_weight = loss_all.mul(weight_mask)
...
#optimizer = optim.SGD(model.parameters(), lr=lr, weight_decay=weight_decay)  #weight_decay = 5e-4  lr = 0.01
#optimizer = torch.optim.Adadelta(model.parameters(), rho=0.9)
optimizer = torch.optim.Adam(model.parameters(),lr=0.001,betas=(0.9, 0.999),eps=1e-08,weight_decay=0,amsgrad=False)
criterion=FocalLoss()
#criterion=Masked_BCELoss() #计算目标值和预测值之间的二进制交叉熵损失函数,返回loss的均值
...
for e in range(epochs):  # 循环训练
    ...
    for protein in train_dataset:
        ...
        E = pickle.load(open(baker_attention_add+pdb_id+"_row_attentions.pkl", 'rb'))[:L,L:]
        #E归一化
    ...
    for protein in val_dataset:
        pdb_id = protein["Complex_code"]
        L = protein["l_length"]
        E = pickle.load(open(ecoil_attention_add + pdb_id + "_row_attentions.pkl", 'rb'))[:L, L:]
        # E归一化
        for E_feature_num in range(np.size(E, 2)):
            E[:, :, E_feature_num] = normalization(E[:, :, E_feature_num])
        Map_Label = protein["Map_Label"][:L, L:]
